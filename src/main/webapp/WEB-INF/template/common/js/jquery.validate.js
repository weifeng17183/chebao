...
(function (c) {
	...
	c.extend(c.validator, {defaults:{messages:{}, groups:{}, rules:{}, errorClass:"error", validClass:"valid", errorElement:"label", focusInvalid:true, errorContainer:c([]), errorLabelContainer:c([]), onsubmit:true, ignore:[], ignoreTitle:false, onfocusin:function (a) {
		this.lastActive = a;
	...
	/** 判断autocomplete显示值与隐藏域是否与数组匹配。
	显示文本框要加属性：extendAttr，里面的值是隐藏域的ID
	d的值是数组。eg:member_input.ftl
	**/
	equalToNull : function(a, b, d) {
		var companyId=document.getElementById(b.extendAttr).value;
		var companyName=a;
		var flag=false;
		for(var i=0;i<d.length;i++){
		if(d[i].id==companyId){
		if(d[i].name==companyName){
		flag=true;
		}
		}
		 }
